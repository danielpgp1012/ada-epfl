function create_top_artists_title(){
  var title_tags = d3.select("body").append("h1")
  .attr("class" , "titles")
  .text("Main artists by tag on subset");
}

function create_top_artists(){
...
.attr("id", "div_main_top_artists")
.attr("class", "container")

var div_tags = d3.select("#div_main_top_artists").append("div")
.attr("id", "div_tags_top_artists")
.attr("class", "row tags");

var div_artists = d3.select("#div_main_top_artists").append("div")
.attr("id", "main_artists")
.attr("class", "row artists");

var period_div = [];
for( i = 0; i<period_array.length; i++){
  period_div[i] = d3.select('#main_artists').append("div")
  .attr("id", "p_sub_"+period_array[i].substring(0,4))
  .attr("class", "column")
...
var filename = "data/top_artists_subset/top_artists_tag_period_artist.json";
...
d3.json(filename, function(errortop, top_artist_data){
  ...
  .on("click",function(d){
    ...
    for(i=0; i<d.children.length; i++){
      ...
      period_id = "#p_sub_" + period_name.substring(0,4);
      ...
      .append("img")
      .attr("src", function(d){
        var image_name = artist_file(d.name);
        return "data/im_resized/" + image_name + ".jpg";
      })
      .attr("width", 100)
      .attr("height", 100);
    };
...
function cleanPeriod(){
  for( i = 0; i<period_div.length; i++){
    d3.select("#p_sub_"+period_array[i].substring(0,4)).selectAll("div:not(.column-title)").remove("*");;
  }
